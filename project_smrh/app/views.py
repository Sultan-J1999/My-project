...
def view_transfer(request):
    if request.method == "POST":
        try:
            ...
            Mywarehouses = Warehouse.objects.all()
            clas = "form-control is-invalid"
            result = "Sorry, Error in select"


            return render(request,'select_warehouse.html',{"Mywarehouses": Mywarehouses, "result": result, "clas": clas})
...
def add_warehouse(request):
    if request.method == "POST":
        ...
        if Warehouse.objects.filter(warehouse_name=warehouse_name).exists():
            ...
            print("done")
            print("done")
            return render(request, "view_add_warehouse.html", {"result": result,"admin_account_date":admin_account_date})
        else:
...
def view_add_warehouse(request):
    ...
    return render(request,'view_add_warehouse.html',{"admin_account_date":admin_account_date})
        ...
        if (
            ...
            return render(
                ...
                "home_page_employee.html",
                {
                    ...
                    "filtered_materials": filtered_materials,
                    "story_name": story_name,'store_name':warehouse_id
                },
            )
        else:
            result = "missing fields"
...
def add(request):
    if request.method == "POST":
        ...
        if employee_type=='Manager':
            x = Employee(name=name, phone=phone, code=int(xx), warehouse_id=warehouse_name)
        else:
        ...
        try:
            ...
            if Employee.objects.filter(code=int(xx)).exists() or Employee.objects.filter(code=code).exists():
                result = "The Code Already Added"
                clas = "form-control is-invalid"
                print("done")
            else:
        ...
        if Admin_Account.objects.filter(admin_email=email).exists():  # admin
            ...
            if request.method == "POST":
                ...
                if Admin_Account.objects.filter(admin_email=email).exists():
                    ...
                    from_email = settings.EMAIL_HOST_USER
                    to_email = [from_email, email]
                    code_message = [random.randint(0, 9) for _ in range(6)]
                    split_code_message = "".join(str(num) for num in code_message)
                    try:
                        send_mail(subject, split_code_message, from_email, to_email)
                        return render(
...
def delete_warehouse(request):
    ...
    # if Section.objects.filter(warehouse_id=warehouse_id).count()==0:
    #     result='there are sections that cannot be delete the warehouse'
    if Material.objects.filter(warehouse_id=warehouse_id).exists() or Section.objects.filter(warehouse_id=warehouse_id).exists():
        result = "Sorry, there are materials or Sections that cannot be deleted the warehouse"

    else:
        x.delete()
        result = ""
    return render(request, "XX.html", {"Mywarehouses": Mywarehouses, "result": result,"admin_account_date":admin_account_date})
...
def generate_rebort(request):
    ...
    report_content += f"1) Total number of materials in the warehouse: {material_count}\n\n"

    if today_materials.exists():
        report_content += "2) Details of materials added today:\n"
        for material in today_materials:
            report_content += f"    Material Name: {material.material_name}\n"
            report_content += f"    Count: {material.count}\n"
            report_content += f"    Count Type: {material.count_type}\n"
        ...
        report_content += "2) No materials have been added today.\n\n"

    if last_added_material:
        report_content += "3) Details of the last added material:\n"
        report_content += f"    Material Name: {last_added_material.material_name}\n"
        report_content += f"    Count: {last_added_material.count}\n"
        report_content += f"    Count Type: {last_added_material.count_type}\n"
        report_content += f"    Date: {last_added_material.date}\n"
        ...
        report_content += "3) No materials have been added to the warehouse.\n"
    ...
    if request.method == "POST":
        ...
        return render(
            ...
            "taking_out_material_employee.html",
            {
                ...
                "date":date,
                "from_section":from_section,
                "employee_code": employee_code,'warehouse_id':warehouse_id
            },
        )
...
# def lang(request):
#     select_lang=request.POST['select_lang']
#     request.session['django_language']=select_lang
#     activate(select_lang)
#     return redirect('/')
# -------------------------------------------------------------------
...
def all_actionsDB(request):
    # warehouse_id=request.POST['warehouse_id']
    try:
        print('yes')
        contennt=''
        activaties=LogEntry.objects.filter(content_type__model='material')
        for i in activaties:
            contennt +=f' {i.action_flag} '+ f' {i.user} '+f' {i.object_repr} '+f' {i.action_time}\n'
            # print(Material.objects.get(id = i.content_type_id).material_name)
        file_path = os.path.expanduser("~/Desktop/ALL_ACTION.txt")
        with open(file_path, 'w') as file:
            file.write(contennt)
        Mywarehouses = Warehouse.objects.all()
        admin_account_date=request.POST['admin_account_date']
...
def employee_profile(request, employee_id):
    ...
    # Pass the employee data to the profile template
...
def update_code(request):
    if request.method=='POST':
        ...
        myemployee = Employee.objects.filter(id=old_id)
        for i in myemployee:
            myemployee.update(code=int(new_code))
        employee = get_object_or_404(Employee, id=old_id)
...
def delete_material(request):
    if request.method == "POST":
        ...
        id_delete=request.POST['id_delete']
        object_delete = Material.objects.get(id=id_delete)
        if id_delete:
            object_delete.delete()
        return render(
            request,
